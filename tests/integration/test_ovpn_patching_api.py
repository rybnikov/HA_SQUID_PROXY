...
def mock_manager():
    ...
    manager = MagicMock()
    manager.list_instances = MagicMock(return_value=[
        {
            "name": "squid-proxy",
            "port": 3128,
            "proxy_type": "squid",
...
class TestPatchOVPNSquidInstance:
    ...
    async def test_patch_ovpn_squid_instance_no_auth(
        ...
        form = FormData()
        form.add_field('file', basic_ovpn_content, filename='client.ovpn', content_type='text/plain')

        # Create mock request with multipart reader
        request = make_mocked_request("POST", "/api/instances/squid-proxy/patch-ovpn")
        request.app = mock_app
        request.match_info = {"name": "squid-proxy"}
        ...
        request.multipart = mock_multipart
    ...
    @pytest.mark.asyncio
    async def test_patch_ovpn_squid_with_auth(
        self, basic_ovpn_content, mock_app, mock_manager
    ):
    ...
    @pytest.mark.asyncio
    async def test_patch_ovpn_with_external_ip(
        self, basic_ovpn_content, mock_app, mock_manager
    ):
...
class TestPatchOVPNTLSTunnelInstance:
    ...
    async def test_patch_ovpn_tls_tunnel_instance(
        ...
        request = make_mocked_request("POST", "/api/instances/tls-tunnel/patch-ovpn")
        request.app = mock_app
        request.match_info = {"name": "tls-tunnel"}
...
class TestPatchOVPNErrorHandling:
    ...
    async def test_patch_ovpn_nonexistent_instance(self, mock_app, mock_manager):
        ...
        request = make_mocked_request("POST", "/api/instances/nonexistent/patch-ovpn")
        request.app = mock_app
