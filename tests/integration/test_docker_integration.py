...
class TestProxyInstanceManager:
    ...
    def test_manager_initialization(self, docker_client):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch("proxy_manager.DATA_DIR", tmp_path), patch(
                ...
                (tmp_path / "squid_proxy_manager" / "logs").mkdir(parents=True)
    ...
    def test_manager_docker_client_ping(self, docker_client):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch("proxy_manager.DATA_DIR", tmp_path), patch(
                ...
                ), "Manager's Docker client should be able to ping"
    ...
    @pytest.mark.asyncio
    async def test_create_instance_basic(
        self, proxy_manager, test_instance_name, test_port, squid_image_available
    ):
        """Test creating a basic proxy instance."""
        if not squid_image_available:
            pytest.fail(
                "Squid proxy image 'squid-proxy-manager' not found.\n"
                "This test requires the Squid Docker image to be built.\n"
    ...
    @pytest.mark.asyncio
    async def test_start_stop_instance(
        self, proxy_manager, test_instance_name, test_port, squid_image_available
    ):
    ...
    async def test_remove_instance(
        ...
        # Create instance using the fixture (which handles temp directories)
        await proxy_manager.create_instance(
            name=test_instance_name, port=test_port, https_enabled=False, users=[]
        )
...
class TestServerIntegration:
    ...
    @pytest.mark.asyncio
    async def test_api_create_instance_integration(
        self, app_with_manager, test_instance_name, test_port, squid_image_available
    ):
