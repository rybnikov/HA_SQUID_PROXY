...
@pytest.mark.asyncio
async def test_create_instance_with_dpi_prevention(mock_popen, temp_data_dir):
    """Test creating a proxy instance with DPI prevention enabled."""
    with (
        patch("proxy_manager.DATA_DIR", temp_data_dir),
        patch("proxy_manager.CONFIG_DIR", temp_data_dir / "squid_proxy_manager"),
        patch("proxy_manager.CERTS_DIR", temp_data_dir / "squid_proxy_manager" / "certs"),
        ...
        assert instance["name"] == "dpi-instance"
        assert instance["dpi_prevention"] is True
        assert instance["status"] == "running"

        # Verify metadata stored dpi_prevention
        metadata_file = temp_data_dir / "squid_proxy_manager" / "dpi-instance" / "instance.json"
        assert metadata_file.exists()
        metadata = json.loads(metadata_file.read_text())
        assert metadata["dpi_prevention"] is True

        # Verify squid.conf contains DPI prevention directives
        config_file = temp_data_dir / "squid_proxy_manager" / "dpi-instance" / "squid.conf"
...
@pytest.mark.asyncio
async def test_create_instance_without_dpi_prevention(mock_popen, temp_data_dir):
    """Test creating a proxy instance with DPI prevention disabled (default)."""
    with (
        patch("proxy_manager.DATA_DIR", temp_data_dir),
        patch("proxy_manager.CONFIG_DIR", temp_data_dir / "squid_proxy_manager"),
        patch("proxy_manager.CERTS_DIR", temp_data_dir / "squid_proxy_manager" / "certs"),
        ...
        # Verify squid.conf does NOT contain DPI prevention directives
        config_file = temp_data_dir / "squid_proxy_manager" / "no-dpi-instance" / "squid.conf"
        content = config_file.read_text()
