...
@pytest.mark.e2e
@pytest.mark.asyncio
async def test_create_form_invalid_forward_address_shows_error(
    browser, unique_name, unique_port
):
    ...
    try:
        ...
        await fill_textfield_by_testid(
            page, "create-port-input", str(unique_port(3200))
        )
        await fill_textfield_by_testid(
            page, "create-forward-address-input", "host with spaces:443"
        )
...
async def test_settings_form_invalid_forward_address_shows_error(
    ...
    try:
        ...
        await fill_textfield_by_testid(
            page, "settings-forward-address-input", "invalid/path:443"
        )
...
async def test_settings_form_invalid_port_shows_error(
    ...
    try:
        ...
        await page.click(f'[data-testid="instance-card-{instance_name}"]')
        await page.wait_for_selector(
            '[data-testid="settings-port-input"]', timeout=30000
        )
